                ...
                <div class="card-body">
                  ...
                  @if($user->employee->cni_number)
                    <div class="button-click-action mb-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon mr-2 text-muted opacity-50" viewBox="0 0 24 24" width="24"
                            height="24">
                            <path fill="none" d="M0 0h24v24H0z" />
                            <path
                    ...
                    <div class="modal-body">
                        <div>
                            <div class="col-lg-12 mb-4">
                                ...
                                <input type="number"id="salary" min="0" value="{{$user->employee->salary }}" class="form-control">
                                <span class="text-danger" id="salary-error"></span>
                            </div>
                            <div class="col-lg-12 mb-4">
                                <label class="form-label"> Début de paiement </label>
                                <input type="date" min="1" max="28" value="{{($user->employee->start_payment) ? $user->employee->start_payment : ''}}" id="start_payment" min="0" class="form-control">
                                <span class="text-danger" id="start_payment-error"></span>
                            </div>
                            <div class="col-lg-12 mb-4">
                                <label class="form-label"> Mode de paiement </label>
                                <select name="role" id="paying_method" class="form-select">
                                    <option {{($user->employee->paying_method == 'cash') ? 'selected' : ''}} value="cash"> Cash </option>
                                    <option {{($user->employee->paying_method == 'om') ? 'selected' : ''}} value="om"> OM </option>
                                    <option {{($user->employee->paying_method == 'momo') ? 'selected' : ''}} value="momo"> MoMo </option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" onclick="updateSalary({{$user->id}})" style="width: 100%">Sauvegarder</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        function updateSalary(user){
           var form_data = new FormData(); // Creating object of FormData class
            ...
            form_data.append("start_payment", $("#start_payment").val());
            form_data.append("paying_method", $("#paying_method").val());
            form_data.append("_token", '{{csrf_token()}}');

            $.ajax({
                url : '/admin/users/'+user+'/salary',
                cache: false,
                contentType: false,
                processData: false,
                method : 'post',
